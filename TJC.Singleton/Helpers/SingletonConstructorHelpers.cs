...
public static class SingletonConstructorHelpers
{
    ...
    public static ConstructorInfo GetSingletonConstructor(Type type)
    {
        ...
        if (publicConstructors.Count != 0)
            throw new InvalidSingletonConstructorException(
                $"[{type}] singleton should not have public constructor{(publicConstructors.Count > 1 ? "s" : string.Empty)}"
            );

        // Ensure there is a non-public parameterless constructor
        var ctor =
            type.GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic, null, [], null)
            ?? throw new InvalidSingletonConstructorException(
                $"[{type}] singleton is missing a non-public parameterless constructor"
            );
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    public static bool HasValidSingletonConstructor<T>() => HasValidSingletonConstructor(typeof(T));
